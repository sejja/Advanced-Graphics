...
vec2 CalcTexCoord() {
   return gl_FragCoord.xy / vec2(1024, 1024);
}
...
void main() {     
    ...
    const vec3 fragPos = texture(gPosition, CalcTexCoord()).rgb;
    const vec3 normal = texture(gNormal, CalcTexCoord()).rgb;
    ...
    FragColor = texture(gAlbedoSpec, CalcTexCoord()) * vec4(totalLightShine, 1.0);
} 