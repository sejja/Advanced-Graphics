...
#pragma region //Functions
			...
			DONTDISCARD constexpr static TYPE inline* reallocate(TYPE* const mem, const size_t size) noexcept;
			static void inline deallocate(TYPE* const data) noexcept;
			DONTDISCARD constexpr static TYPE inline* New(const size_t count = 1) noexcept;

			template<typename ...Args>
			static void inline construct(TYPE* const data, Args&& ...ag) noexcept;
			static void inline destroy(TYPE* const data) noexcept;
#pragma endregion
		};
		...
		*/ //--------------------------------------------------------------------
		template<typename TYPE>
		constexpr TYPE inline* Allocator<TYPE>::reallocate(TYPE* const mem, const size_t size) noexcept {
		...
		*/ //----------------------------------------------------------------------
		template<typename TYPE>
		void inline Allocator<TYPE>::deallocate(TYPE* const data) noexcept {
			free(data);
		}
		...
		*/ //----------------------------------------------------------------------
		template<typename TYPE>
		constexpr TYPE inline* Allocator<TYPE>::New(const size_t count) noexcept {
		...
		*/ //-------------------------------------------------------------------
		template<typename TYPE>
		template<typename ...Args>
		void inline Allocator<TYPE>::construct(TYPE* const data, Args&& ...ag) noexcept {
			new(data) TYPE(ag...);
		}
		...
		*/ //-------------------------------------------------------------------
		template<typename TYPE>
		void inline Allocator<TYPE>::destroy(TYPE* const data) noexcept {
			data->~TYPE();
		}
	}
}
